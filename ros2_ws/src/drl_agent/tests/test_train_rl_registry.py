...
in a FRESH subprocess rather than importing it in-process, for two reasons:
  ...
  2. Several sibling ROS-free tests in this same suite install permanent,
     ...
     "drl_agent.env.environment_interface"; several others stub "rclpy"
     itself). Those stubs stay in sys.modules for the rest of the pytest
     session. An in-process import of train_rl was observed to fail
     intermittently depending on which other tests happened to run first and
     leave a stub behind (a stub `rclpy` broke `from rclpy.node import Node`
     with a confusing `TypeError: '_Meta' object is not iterable`, for one).
     A subprocess never inherits that pollution, so these tests are correct
...
built. Every import here is the canonical dotted path
(``drl_agent.training.train_rl``) — there is no bare-name fallback.
"""
...
_TRAIN_RL = os.path.normpath(os.path.join(
    os.path.dirname(__file__), "..", "drl_agent", "training", "train_rl.py"))
...
def _probe_have_ros():
    proc = _run_snippet("import rclpy, drl_agent.training.train_rl")
...
def test_resolve_tqc_family_models_are_importable_and_callable():
    ...
    covers every registered model whose sibling module has no third-party
...
def test_resolve_sb3_models_either_import_cleanly_or_fail_on_their_own_deps():
    """The sb3_* entries are real, on-disk modules (see the disk-existence
...
def test_resolve_rl_model_name_priority_cli_ros_param_default():
    code = (
        "import rclpy, drl_agent.training.train_rl as train_rl\n"
        "rclpy.init(args=[])\n"
