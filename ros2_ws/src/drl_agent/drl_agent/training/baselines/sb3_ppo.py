...
from drl_agent.rl.algorithms.sb3.ppo import SB3PPOAgent as Agent
from drl_agent.env.environment_interface import EnvInterface
from drl_agent.common.file_manager import DirectoryManager, load_yaml


class TrainSB3PPO(EnvInterface):
    def __init__(self):
        """Initialize SB3 PPO training node"""
        super().__init__("train_sb3_ppo_node")
        ...
        # ----------------------------
        train_cfg_path = self._find_config_file("train_sb3_ppo_config.yaml", user_param_path)
        if not train_cfg_path:
            self.get_logger().error("Could not find 'train_sb3_ppo_config.yaml'")
            raise FileNotFoundError("train_sb3_ppo_config.yaml not found")
        ...
        self.use_checkpoints = False  # PPO is on-policy, no checkpoint mode
        self.eval_freq = train_settings["eval_freq"]
        self.timesteps_before_training = 0  # PPO is on-policy: no warmup
        ...
        # ----------------------------
        hparams_path = self._find_config_file("hyperparameters_sb3_ppo.yaml", user_param_path)
        if not hparams_path:
            self.get_logger().error("Could not find 'hyperparameters_sb3_ppo.yaml'")
            raise FileNotFoundError("hyperparameters_sb3_ppo.yaml not found")

        # SB3 config files have a flat YAML structure (no ["hyperparameters"] key)
        hyperparameters = load_yaml(hparams_path)

        # ----------------------------
        # Create SB3 PPO agent
        # ----------------------------
        self.rl_agent = Agent(
            state_dim,
            action_dim,
    ...
    def _setup_directories(self):
        ...
        if run_dir_param:
            ...
            base_run_dir = os.path.join(package_root, "runtime", "sb3_ppo")
    ...
    def log_training_setting_data(self):
        ...
        self.get_logger().info("SB3 PPO Training Configuration")
        self.get_logger().info("=" * 50)
    ...
    def train_online(self):
        """Main training loop (on-policy PPO)"""
        start_time = time.time()
        evals = []
        epoch = 1
        timesteps_since_eval = 0

        # PPO is on-policy: training starts immediately (no warmup)
        allow_train = True
        ...
        self.get_logger().info("Starting SB3 PPO training...")
        ...
        for t in range(1, self.max_timesteps + 1):
            ...
            # Select action (PPO DiagGaussian: returns raw Normal sample)
            # action_raw is stored in rollout buffer for exact log_prob recomputation.
            # action_env is clipped to [-1, 1] for environment interaction.
            action_raw, log_prob, value = self.rl_agent.select_action(state)
            action_env = np.clip(action_raw, -1.0, 1.0)

            # Environment step (clipped action)
            next_state, reward, ep_finished, info = self.step(action_env)

            # Timeout penalty
            if ep_timesteps == self.max_episode_steps - 1 and not ep_finished:
                reward -= 20.0

            # Add raw action to rollout buffer (on-policy)
            done_flag = float(ep_finished) if ep_timesteps < self.max_episode_steps else 0.0
            self.rl_agent.add_rollout(state, action_raw, reward, done_flag, value, log_prob)
            ...
            # Accumulate per-step driving data (use env action for logging)
            action = action_env
            ...
            # PPO update: train when rollout buffer is full
            if self.rl_agent.rollout_full():
                last_value = self.rl_agent.get_value(state)
                last_done  = float(ep_finished)
                self.rl_agent.train(last_value, last_done)
                self.rl_agent.clear_rollout()
    ...
    def evaluate_and_print(self, evals, epoch, start_time):
        ...
        for ep in range(self.eval_eps):
            ...
            while not done and ep_timesteps < self.max_episode_steps:
                action_raw, _, _ = self.rl_agent.select_action(state, deterministic=True)
