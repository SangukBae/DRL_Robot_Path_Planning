...
from drl_agent.rl.algorithms.sb3.sac import SB3SACAgent as Agent
...
class TrainSB3SAC(EnvInterface):
    def __init__(self):
        """Initialize SB3 SAC training node"""
        super().__init__("train_sb3_sac_node")
        ...
        # ----------------------------
        train_cfg_path = self._find_config_file("train_sb3_sac_config.yaml", user_param_path)
        if not train_cfg_path:
            self.get_logger().error("Could not find 'train_sb3_sac_config.yaml'")
            raise FileNotFoundError("train_sb3_sac_config.yaml not found")
        ...
        # ----------------------------
        hparams_path = self._find_config_file("hyperparameters_sb3_sac.yaml", user_param_path)
        if not hparams_path:
            self.get_logger().error("Could not find 'hyperparameters_sb3_sac.yaml'")
            raise FileNotFoundError("hyperparameters_sb3_sac.yaml not found")
    ...
    def _setup_directories(self):
        ...
        if run_dir_param:
            ...
            base_run_dir = os.path.join(package_root, "runtime", "sb3_sac", f"seed_{self.seed}")
    ...
    def log_training_setting_data(self):
        ...
        self.get_logger().info("SB3 SAC Training Configuration")
        self.get_logger().info("=" * 50)
    ...
    def train_online(self):
        ...
        self.get_logger().info("Starting SB3 SAC training...")
