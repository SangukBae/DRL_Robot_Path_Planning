...
from drl_agent.rl.algorithms.td7.agent import Agent
...
class TrainTD7(EnvInterface):
    def __init__(self):
        ...
        train_cfg_path = self._find_config_file("train_td7_config.yaml", user_param_path)
        if not train_cfg_path:
            self.get_logger().error("Could not find 'train_td7_config.yaml'")
            raise FileNotFoundError("train_td7_config.yaml not found")
        ...
        hparams_path = self._find_config_file("hyperparameters_td7.yaml", user_param_path)
        if not hparams_path:
            self.get_logger().error("Could not find 'hyperparameters_td7.yaml'")
            raise FileNotFoundError("hyperparameters_td7.yaml not found")
    ...
    def _setup_directories(self):
        ...
        if run_dir_param:
            ...
            base_run_dir = os.path.join(package_root, "runtime", "td7", f"seed_{self.seed}")
    ...
    def log_training_setting_data(self):
        ...
        self.get_logger().info("TD7 Training Configuration")
        self.get_logger().info("=" * 50)
    ...
    def train_online(self):
        ...
        self.get_logger().info("Starting TD7 training...")
