...
from drl_agent.rl.algorithms.a3c.agent import Agent
...
class TrainA3C(EnvInterface):
    def __init__(self):
        ...
        train_cfg_path = self._find_config_file("train_a3c_config.yaml", user_param_path)
        if not train_cfg_path:
            self.get_logger().error("Could not find 'train_a3c_config.yaml'")
            raise FileNotFoundError("train_a3c_config.yaml not found")
        ...
        hparams_path = self._find_config_file("hyperparameters_a3c.yaml", user_param_path)
        if not hparams_path:
            self.get_logger().error("Could not find 'hyperparameters_a3c.yaml'")
            raise FileNotFoundError("hyperparameters_a3c.yaml not found")
        ...
        def _find_latest_prefix(models_dir, base, seed):
            import glob
            pat = os.path.join(models_dir, f"{base}_seed_{seed}_*_actor_critic.pth")
            cands = glob.glob(pat)
            if not cands:
                return None
            cands.sort(key=lambda p: os.stat(p).st_mtime, reverse=True)
            return os.path.basename(cands[0]).replace("_actor_critic.pth", "")
    ...
    def _setup_directories(self):
        ...
        if run_dir_param:
            ...
            base_run_dir = os.path.join(package_root, "runtime", "a3c")
    ...
    def log_training_setting_data(self):
        ...
        self.get_logger().info("A3C Training Configuration")
        self.get_logger().info("=" * 50)
    ...
    def train_online(self):
        ...
        self.get_logger().info("Starting A3C training...")
        ...
        for t in range(1, self.max_timesteps + 1):
            ...
            if self.rl_agent.replay_buffer is not None:
