#!/usr/bin/env python3
"""Curriculum-learning subclass of TrainSB3TD3.

Same curriculum mechanism as drl_agent.training.baselines.sac_curriculum:
  - Loads curriculum_settings from train_sb3_td3_curriculum_config.yaml
...
shared with SAC/TD7/SB3-SAC via BaselineCurriculumTrainerMixin (see
drl_agent/training/curriculum/trainer_base.py) — this file only supplies the
SB3-TD3-specific config filename/default and its select_action call
...
Usage:
  ros2 run drl_agent train_rl.py --ros-args -p rl_model:=sb3_td3_curriculum
...
from drl_agent.training.baselines.sb3_td3 import TrainSB3TD3
from drl_agent.training.curriculum.trainer_base import BaselineCurriculumTrainerMixin


class TrainSB3TD3Curriculum(BaselineCurriculumTrainerMixin, TrainSB3TD3):
    """SB3 TD3 trainer with automatic curriculum stage advancement.

    Inherits setup, training loop, and model I/O from TrainSB3TD3; stage
    advancement / eval / resume / CSV logging come from
    BaselineCurriculumTrainerMixin (shared with SAC/TD7/SB3-SAC).
    """

    CURRICULUM_CONFIG_FILENAME = "train_sb3_td3_curriculum_config.yaml"
    DEFAULT_MIN_STAGE_STEPS = 30000

    def __init__(self):
        super().__init__()   # TrainSB3TD3.__init__ (calls _init_csv_loggers, from the mixin)
