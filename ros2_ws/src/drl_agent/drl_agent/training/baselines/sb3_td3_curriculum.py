...
class TrainSB3TD3Curriculum(TrainSB3TD3):
    """SB3 TD3 trainer with automatic curriculum stage advancement.

    Inherits all setup, training loop, and model I/O from TrainSB3TD3.
    ...
    def __init__(self):
        ...
        cur_cfg_path = self._find_config_file("train_sb3_td3_curriculum_config.yaml")
        if cur_cfg_path:
            cur = load_yaml(cur_cfg_path).get("curriculum_settings", {})
        else:
            self.get_logger().warn(
                "[Curriculum] train_sb3_td3_curriculum_config.yaml not found — using defaults."
            )
            cur = {}
                ...
                if _ep_v_buf:
                    with open(self._driving_csv, "a", newline="") as _f:
                        csv.writer(_f).writerow([
                            ...
                            float("nan"),  # mean_gazebo_rtf: not tracked by SB3-TD3
                        ])
