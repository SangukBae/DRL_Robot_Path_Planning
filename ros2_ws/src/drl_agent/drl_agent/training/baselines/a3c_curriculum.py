#!/usr/bin/env python3
"""Curriculum-learning subclass of TrainA3C.

Same curriculum mechanism as drl_agent.training.train_tqc_curriculum:
  - Loads curriculum_settings from train_a3c_curriculum_config.yaml
...
Usage:
  ros2 run drl_agent train_rl.py --ros-args -p rl_model:=a3c_curriculum
...
from drl_agent.training.baselines.a3c import TrainA3C
from drl_agent.common.file_manager import load_yaml


class TrainA3CCurriculum(TrainA3C):
    """A3C trainer with automatic curriculum stage advancement.

    Inherits all setup, training loop, and model I/O from TrainA3C.
    ...
    def __init__(self):
        ...
        cur_cfg_path = self._find_config_file("train_a3c_curriculum_config.yaml")
        if cur_cfg_path:
            cur = load_yaml(cur_cfg_path).get("curriculum_settings", {})
        else:
            self.get_logger().warn(
                "[Curriculum] train_a3c_curriculum_config.yaml not found — using defaults."
            )
            cur = {}
