#!/usr/bin/env python3
# NOTE: this is a ROS2 RUN/EVALUATION script (it drives a live Gazebo episode
# and loads a trained actor), NOT a pytest unit test — hence the canonical
# module name has no "test_" prefix (unlike its historical scripts/ filename),
# so a bare `pytest` invocation never mistakes it for a test module. Launched
# via `ros2 run drl_agent td7_live_runner.py ...` (see launch/test_td7.launch.py).
# The fast, ROS-free pytest unit tests live under
# `ros2_ws/src/drl_agent/tests/` (run with `pytest`).
...
from drl_agent.common.file_manager import load_yaml, save_yaml, save_json
