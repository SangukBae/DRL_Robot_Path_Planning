...
def parse_bool_override(raw: str, default_value: bool) -> bool:
    """Parse a string ROS2 parameter used to override a YAML config boolean.

    Callers declare the parameter with an empty-string default, so "no value
    passed" is distinguishable from an explicit true/false (a plain bool
    parameter can't represent "unset" without always winning over the config
    file). Priority: explicit override > ``default_value`` (the value read
    from YAML).

      ""                    -> default_value (config file wins, unchanged)
      "true"/"1"/"yes"/"on"  -> True
      "false"/"0"/"no"/"off" -> False (case-insensitive)

    Used for the PHASE2 risk-shaping flags (``risk_map_reward_enabled`` /
    ``action_risk_head_enabled``) so the 4-way experiment matrix (neither /
    candidate1 / candidate2 / both) can be selected with ``-p key:=value``
    instead of editing environment_curriculum.yaml / hyperparameters_tqc.yaml.
    """
