...
def _pkg_root():
    # scripts/utils/<this> -> package root is two levels up.
...
human RNG is isolated and the global stream is no longer perturbed by humans.
  ...
  3. Compare the EARLY eval metrics of the two runs:
        python3 check_reproducibility.py manifest <run1>/logs   # same git + cfg hashes?
...
To find any op that lacks a deterministic CUDA kernel, flip warn_only off:
    ros2 run drl_agent train_tqc_curriculum_agent.py --ros-args \\
        -p seed:=0 -p deterministic_warn_only:=false
