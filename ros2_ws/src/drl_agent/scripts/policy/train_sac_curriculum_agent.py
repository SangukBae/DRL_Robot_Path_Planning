...
Usage:
  ros2 run drl_agent train_sac_curriculum_agent.py
...
class TrainSACCurriculum(TrainSAC):
    ...
    def __init__(self):
        ...
        cur_cfg_path = self._find_config_file("train_sac_curriculum_config.yaml")
        if cur_cfg_path:
            cur = load_yaml(cur_cfg_path).get("curriculum_settings", {})
        else:
            self.get_logger().warn(
                "[Curriculum] train_sac_curriculum_config.yaml not found — using defaults."
            )
            cur = {}
                ...
                if _ep_v_buf:
                    with open(self._driving_csv, "a", newline="") as _f:
                        csv.writer(_f).writerow([
                            ...
                            float("nan"),  # mean_gazebo_rtf: not tracked by SAC
                        ])
